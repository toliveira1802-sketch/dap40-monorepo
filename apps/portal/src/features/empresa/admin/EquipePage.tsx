...
import { hasProjectFloatAccess } from "./canAccess";
...
/** Só Dev (PORTAL-EMPRESA) / MASTER — implantação e desenvolvimento. */
function RequireDevFloat({ children }: { children: ReactNode }) {
  ...
  if (!session || !hasProjectFloatAccess(session.systems, session.role)) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="AIOS · Dev"
        title="Colaboradores"
        description="Cadastro operacional da equipe — só implantadores / desenvolvedores."
        actions={
          <Button
            onClick={() => {
              setEditing(null);
      ...
      {isError ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="h-44 animate-pulse rounded-2xl bg-card" />
          ))}
        </div>
      ) : team.length ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {team.map(member => (
            <button
              ...
              className="group rounded-2xl border border-border bg-card/80 p-5 text-left shadow-panel transition hover:-translate-y-0.5 hover:border-primary/35"
            >
              ...
              <p className="mt-3 min-h-10 text-sm leading-5 text-muted-foreground">
                {member.specialty || "Especialidade não informada"}
              </p>
              <div className="mt-4 border-t border-border pt-3 text-xs text-muted-foreground">
                {member.phone || member.email || "Contato não informado"}
              </div>
            </button>
          ))}
        </div>
        ...
        <EmptyState
          ...
          description="Cadastre a equipe para atribuir responsáveis no pátio e na operação."
          action={
    ...
    setForm(
      collaborator
        ? {
            ...
            position: (collaborator.position as (typeof COLLABORATOR_POSITIONS)[number]) || "tecnico",
      ...
      <DialogContent className="border-border bg-popover sm:max-w-lg">
        <form onSubmit={submit} className="space-y-5">
          <DialogHeader>
            ...
            <DialogDescription>
              Função exclusiva do portal AIOS (Dev) — implantação e desenvolvimento.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={value =>
                  setForm(current => ({
                    ...current,
                    position: value as typeof form.position,
                  }))
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {COLLABORATOR_POSITIONS.map(position => (
            ...
            {collaborator ? (
              <label className="flex items-center justify-between rounded-xl border border-border p-3 sm:col-span-2">
                <span>
                  <span className="block text-sm font-medium">Colaborador ativo</span>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={pending}>
              {pending ? <Loader2 className="size-4 animate-spin" /> : null} Salvar
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
